    ...
    <div className="max-w-7xl mx-auto p-6 space-y-8">
      <header className="flex justify-between items-center border-b border-slate-800 pb-5">
        <div>
          ...
          <p className="text-sm text-slate-400">
            Ready for a productive distraction-free session?
          </p>
        </div>
        <button
          onClick={logout}
          className="px-4 py-2 text-sm bg-rose-600/20 text-rose-400 border border-rose-500/30 rounded-lg hover:bg-rose-600 hover:text-white transition-all cursor-pointer"
        >
          Logout
        </button>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <form
          ...
          className="bg-brand-panel p-6 rounded-xl border border-slate-800 space-y-4 h-fit"
        >
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {rooms.map((room) => (
              <div
                key={room._id}
                className="bg-brand-panel p-5 rounded-xl border border-slate-800 flex flex-col justify-between hover:border-slate-700 transition-all"
              >
                <div>
                ...
                <div className="mt-5 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="bg-slate-800 hover:bg-indigo-600 text-white px-4 py-1.5 rounded-md text-xs font-medium transition-colors cursor-pointer"
                  >
                    Join Room
                  </button>
                </div>
