  ...
  if (!room)
    return (
      <div className="p-6 text-center text-slate-400">
        Syncing workspace room profile...
      </div>
    );
    ...
    <div className="max-w-7xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-2rem)]">
      <div className="lg:col-span-2 flex flex-col justify-between space-y-6">
        <div className="bg-brand-panel p-6 rounded-xl border border-slate-800 flex justify-between items-center">
          ...
          <button
            onClick={leaveRoom}
            className="text-xs px-3 py-1.5 bg-slate-800 text-white border border-slate-700 rounded-lg hover:bg-slate-700 cursor-pointer"
          >
            Exit Workspace
          </button>
        </div>
        ...
        <div className="bg-brand-panel p-8 rounded-xl border border-slate-800 flex flex-col items-center justify-center flex-1 space-y-6">
          ...
          <button
            onClick={toggleTimer}
            className={`px-8 py-2.5 rounded-full font-bold text-sm shadow-md text-white transition-all cursor-pointer ${isRunning ? "bg-amber-600 hover:bg-amber-500" : "bg-emerald-600 hover:bg-emerald-500"}`}
          >
        ...
        <div className="bg-brand-panel p-5 rounded-xl border border-slate-800 max-h-48 overflow-y-auto">
          <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">
            Workspace Interval History Logs
          </h4>
          <div className="space-y-1 text-xs font-mono text-slate-500">
            {room.history?.map((h, i) => (
              <div key={i}>
                ✓ Tracked session block at{" "}
                {new Date(h.startTime).toLocaleTimeString()} for 25 mins
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-brand-panel rounded-xl border border-slate-800 flex flex-col justify-between h-full overflow-hidden">
        ...
        <div className="p-4 flex-1 overflow-y-auto space-y-3">
          {messages.map((m, idx) => (
            <div
              key={idx}
              className={`p-3 rounded-lg text-sm ${m.sender === user.username ? "bg-indigo-600/20 border border-indigo-500/30 ml-8" : "bg-slate-800/60 border border-slate-700/30 mr-8"}`}
            >
        ...
        >
          ...
          />
          <button className="bg-indigo-600 text-white hover:bg-indigo-500 px-4 py-2 rounded-lg text-sm font-semibold cursor-pointer">
            Send
          </button>
        </form>
      </div>
